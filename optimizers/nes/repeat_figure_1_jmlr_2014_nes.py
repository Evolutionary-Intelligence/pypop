...
class CanonicalSearchGradientAlgorithm(object):
    ...
    def initialize(self, mu=1.0, sigma=np.array([[1.0]])):
        ...
        history = {"mu": [mu], "sigma": [float(sigma)]}
    ...
    def optimize(self):
        ...
        for _ in range(int(self.max_generations)):
            ...
            mu -= self.eta * np.mean(g_mu, 0)
            sigma -= self.eta * np.mean(g_sigma, 0)
...
if __name__ == "__main__":
    def sphere(x):
        return np.power(x, 2)
    for _ in range(1):
        sg = CanonicalSearchGradientAlgorithm(sphere)
        history_mean_std = sg.optimize()
        plt.plot(history_mean_std["mu"], "black")
        plt.plot(history_mean_std["sigma"], "red")
        print(np.min(history_mean_std["mu"]), np.min(history_mean_std["sigma"]))
        print(np.max(history_mean_std["mu"]), np.max(history_mean_std["sigma"]))
    plt.yscale("log")
    plt.show()
