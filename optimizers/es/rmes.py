...
class RMES(R1ES):
    ...
    def initialize(self, args=None):
        x, mu, p, s, y = R1ES.initialize(self, args)
        mp = np.zeros((self.n_evolution_paths, self.ndim_problem))  # multiple evolution paths
        t_hat = np.zeros((self.n_evolution_paths,))
        return x, mu, p, s, mp, t_hat, y

    def iterate(self, x=None, mu=None, p=None, s=None, mp=None, t_hat=None, y=None, args=None):
        for k in range(self.n_individuals):
            ...
            x[k] = mu + self.sigma * (self._a_m * z + self._b * sum_p)
            y[k] = self._evaluate_fitness(x[k], args)
        return x, y

    def _update_distribution(self, x=None, mu=None, p=None, s=None, mp=None, t_hat=None, y=None, y_bak=None):
        mu, p, s = R1ES._update_distribution(self, x, mu, p, s, y, y_bak)
        # update multiple evolution paths
        t_min = np.min(np.diff(t_hat))
        if (t_min > self.generation_gap) or (self.n_generations < self.n_evolution_paths):
            ...
            for i in range(i_apostrophe, self.n_evolution_paths - 1):
                ...
                t_hat[i] = t_hat[i + 1]
        mp[-1] = p
        t_hat[-1] = self.n_generations
        return mu, p, s, mp, t_hat

    def optimize(self, fitness_function=None, args=None):  # for all generations (iterations)
        ES.optimize(self, fitness_function)
        fitness = []  # store all fitness generated during evolution
        x, mu, p, s, mp, t_hat, y = self.initialize(args)
        fitness.append(y[0])
        while True:
            y_bak = np.copy(y)
            x, y = self.iterate(x, mu, p, s, mp, t_hat, y, args)  # sample and evaluate offspring population
            if self.record_fitness:
                fitness.extend(y.tolist())
            if self._check_terminations():
                break
            mu, p, s, mp, t_hat = self._update_distribution(x, mu, p, s, mp, t_hat, y, y_bak)
