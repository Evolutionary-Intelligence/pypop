"""Compare computational efficiency between *DEAP (SES)* and *PYPOP7 (SES)*
...
def read_pickle(s, ii):
    with open(os.path.join('./docs/efficiency/sphere', s + '-SES_' + ii + '.pickle'), 'rb') as handle:
...
if __name__ == '__main__':
    ...
    plt.yscale('log')
    plt.savefig('./figs/compare_deap-ses_vs_pypop7-ses[cost].eps')
    # plt.show()

    sns.set_theme(style='dark')
    algos = ['DEAP', 'PYPOP7']
    labels = ['DEAP (SES)', 'PyPop7 (SES)']
    colors = ["#F08C55", "#6EC8C8"]
    max_runtime, fitness_threshold = 3600 * 3 - 10 * 60, 1e-10
    time, fitness, fe = [], [], []
    for j in range(len(algos)):  # initialize
    ...
    plt.title('Sphere', fontsize=30, fontweight='bold')
    plt.savefig('./figs/compare_deap-ses_vs_pypop7-ses[fe].eps')
    # plt.show()
