...
def read_pickle(s, ii):
    file_name = os.path.join('./', s + '-PSO_' + ii + '.pickle')
...
if __name__ == '__main__':
    ...
    plt.xticks([0e7, 2e7, 4e7, 6e7, 8e7],
               ['0e7', '2e7', '4e7', '6e7', '8e7'])
    plt.title('Sphere')
    plt.legend(labels)
    plt.yscale('log')
    plt.savefig('compare_deap-pso_vs_pypop7-spso[cost].png',
    ...
    ax1.set_yticks([0e7, 2e7, 4e7, 6e7, 8e7],
                   ['0e7', '2e7', '4e7', '6e7', '8e7'])
    ...
    ax2.set_yticks(np.arange(0, 19, 3), np.arange(0, 19, 3))
    plt.title('Sphere')
    plt.savefig('compare_deap-pso_vs_pypop7-spso[fe].png',
                dpi=700, bbox_inches='tight')
    plt.show()
