...
def read_pickle(s, ii):
    file_name = os.path.join('./', s + '-DE_' + ii + '.pickle')
...
if __name__ == '__main__':
    ...
    plt.ylim([6.3e4, 6.7e4])
    plt.yticks([6.3e4, 6.4e4, 6.5e4, 6.6e4, 6.7e4],
    ...
    plt.xticks([0.0e8, 0.4e8, 0.8e8, 1.2e8],
                   ['0.0e8', '0.4e8', '0.8e8', '1.2e8'])
    plt.title('Sphere')
    plt.legend(labels)
    plt.savefig('compare_deap-de_vs_pypop7-cde[cost].png',
    ...
    ax1.set_yticks([0.0e8, 0.4e8, 0.8e8, 1.2e8],
                   ['0.0e8', '0.4e8', '0.8e8', '1.2e8'])
    ...
    ax2.set_yticks(np.arange(0, 11, 1), np.arange(0, 11, 1))
    plt.title('Sphere')
    plt.savefig('compare_deap-de_vs_pypop7-cde[fe].png',
                dpi=700, bbox_inches='tight')
    plt.show()
