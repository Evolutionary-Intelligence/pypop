"""This Python script plots the *median* convergence curves for various PSOs
    with respective to number of function evaluations.
...
if __name__ == '__main__':
    ...
    n_trials = 10
    algos = ['DEAPPSO', 'SPSO', 'SPSOL', 'CPSO', 'CLPSO', 'IPSO', 'CCPSO2']
    ...
    for k, f in enumerate(funcs):
        ...
        plt.yscale('log')
        for j, a in enumerate(algos):
            plt.plot(fe[j][top_order[j]], fitness[j][top_order[j]], label=a)
        ...
        plt.savefig('./figures/PSOs-' + f + '-FEs.png', format='png')
        plt.show()
